...
import doc_generator as docgen  # Import backend module
...
class App(tk.CTk):
    def __init__(self):
        ...
        x = int((screen_w / 2) - (self.windims[0] / 2))
        y = int((screen_h / 2) - (self.windims[1] / 2)) - 35
        self.geometry(f"{self.windims[0]}x{self.windims[1]}+{x}+{y}")
        self.resizable(False, False)
        self.title("Report Generator")

        icon_path = ImageTk.PhotoImage(file="v1/icon.png")
    ...
    def go_next(self):
        self.save_current_inputs()
        docgen.replace_bookmarks(self.user_inputs[self.current_page])  # Update doc with current page's data
