...
def insert_static_content():
    ...
    # Title placeholder (as bookmark)
    word.Selection.TypeText("___\n")
    title_range = word.Selection.Range.Duplicate
    title_range.MoveStart(Unit=c.wdCharacter, Count=-4)  # Go back to cover "___"
...
def replace_bookmarks(data_dict: dict):
    ...
    for key, value in data_dict.items():
        ...
        if doc.Bookmarks.Exists(key):
            ...
            bm_start = bm_range.Start  # Store position before it's gone
            bm_range.Text = value + "\n"

            # Recreate a new range
            new_range = doc.Range(bm_start, bm_start + len(value))
    ...
    # Move to end for continued work
    cursor = doc.Range()
    cursor.Collapse(c.wdCollapseEnd)
    cursor.Select()
