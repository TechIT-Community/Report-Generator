...
class App(tk.CTk):
    ...
    def generate_report(self):
        ...
        for page_index, page in enumerate(self.user_inputs):
            final_report += f"--- Page {page_index + 1} ---\n"
            for key, value in page.items():
                final_report += f"{key}: {value}\n"
            final_report += "\n"

        # Show live preview 
        self.show_preview_window(final_report)

    def show_preview_window(self, report_text: str):
        preview_window = tk.CTkToplevel(self)
        preview_window.title("Live Report Preview")
        preview_window.geometry("600x600")
        preview_window.after(200, lambda : preview_window.focus())  # Lock focus on preview

        heading = tk.CTkLabel(preview_window, text="Report Preview", font=("Arial", 20, "bold"))
        heading.pack(pady=10)

        text_frame = tk.CTkFrame(preview_window)
        text_frame.pack(pady=10, padx=10, fill="both", expand=True)

        textbox = tk.CTkTextbox(text_frame, wrap="word")
